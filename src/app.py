...
from utils import white_noise_generator, generate_video_frames, generate_audio_stream
from settings import APP_PORT, white_noise_volume
...
def toggle_led(led_id):
    ...
    if led_id in led_states:
        ...
        # Here you would add actual GPIO control for Raspberry Pi
        # Example: GPIO.output(LED_PIN_MAP[led_id], led_states[led_id])
        
        return jsonify({
            'success': True,
            'led': led_id,
            'state': led_states[led_id]
        })
...
def start_white_noise():
    ...
    if not white_noise_generator.playing:
        ...
        thread = threading.Thread(target=white_noise_generator.generate, args=(white_noise_volume,))
        thread.daemon = True
        thread.start()
